...
export function PublicMenuSection({ menu }: { menu: PublicMenu }) {
  ...
  return (
    <View style={styles.section}>
      ...
      {!populated.length && (
        <Text style={styles.stateText}>This shop has not published available menu items yet.</Text>
      )}
          ...
          {category.items.map((item) => (
            <View key={item.id} style={styles.menuItem}>
              ...
              <View style={styles.menuCopy}>
                ...
                {item.event ? <Text style={styles.menuDescription}>{item.event.message}</Text> : null}
