...
export function ContentDetailScreen({ item, onBack, onMoreFromBusiness }: { item: ContentDetail; onBack: () => void; onMoreFromBusiness: (businessId: string, businessName: string) => void }) {
  const follow = useBusinessFollow(item.businessId);
  ...
  return <ScrollView contentContainerStyle={{ paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
    {item.coverUrl && <Image source={{ uri: item.coverUrl }} style={styles.detailHero} />}
    <View style={styles.detailBody}>
      ...
      <View style={styles.sectionCard}>
        ...
        {follow.loading ? <ActivityIndicator color={palette.green} /> : follow.following ? <>
          <View style={styles.switchRow}><View style={styles.switchCopy}><Text style={styles.switchTitle}>Event alerts</Text><Text style={styles.switchHint}>Receive reminders, updates, and cancellations from this shop.</Text></View><Switch disabled={follow.busy} value={follow.eventNotificationsEnabled} onValueChange={(value) => run(() => follow.setAlerts(value))} trackColor={{ false: '#D7CEC6', true: palette.mint }} thumbColor={follow.eventNotificationsEnabled ? palette.green : palette.muted} /></View>
          <View style={[styles.actions, { marginTop: 12 }]}><Pressable onPress={() => onMoreFromBusiness(item.businessId, item.businessName)} style={styles.primaryButton}><Text style={styles.primaryText}>More from this shop</Text></Pressable><Pressable disabled={follow.busy} onPress={() => run(follow.unfollow)} style={[styles.secondaryButton, styles.warningButton]}><Text style={[styles.secondaryText, styles.warningText]}>Unfollow</Text></Pressable></View>
        </> : <Pressable disabled={follow.busy} onPress={() => run(follow.follow)} style={[styles.primaryButton, { marginTop: 12 }]}>{follow.busy ? <ActivityIndicator color={palette.paper} /> : <><Ionicons name="person-add-outline" size={18} color={palette.paper} /><Text style={styles.primaryText}>Follow shop and enable alerts</Text></>}</Pressable>}
