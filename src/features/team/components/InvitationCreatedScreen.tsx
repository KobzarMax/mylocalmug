...
export function InvitationCreatedScreen({
  invitation,
  onBack,
  onDone,
}: {
  ...
  const copyCode = () =>
    clipboard.copy(invitation.token).catch((caught) => {
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.inviteBanner}>
          <Text style={styles.inviteTitle}>Share this code securely</Text>
          <Text style={styles.inviteText}>
            Send it only to {invitation.email}. They must sign in with that exact email and open the Business
            Portal.
          </Text>
        </View>
        <View style={styles.tokenCard}>
          ...
          >
            <Ionicons
              name={clipboard.copied ? 'checkmark' : 'copy-outline'}
              size={18}
              color={palette.green}
            />
          ...
          <Text style={styles.warning}>
            This is the only time the full code is shown. It expires{' '}
            {new Date(invitation.expiresAt).toLocaleString()}.
          </Text>
        </View>
        <Pressable accessibilityRole="button" onPress={onDone} style={styles.primaryButton}>
          <Text style={styles.primaryText}>Return to team</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
