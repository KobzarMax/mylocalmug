...
type Screen = { type: 'overview' } | { type: 'editor'; item: ContentItem | null; kind?: 'news' | 'event' } | { type: 'cancel'; item: ContentItem };
...
export function BusinessContentEntry({ workspace, onBack }: { workspace: Workspace; onBack: () => void }) {
  ...
  const saved = async () => { setScreen({ type: 'overview' }); await content.refresh(); };
  if (screen.type === 'editor') return <ContentEditorScreen
    ...
    onSaved={saved}
  />;
  if (screen.type === 'cancel') return <CancelEventScreen item={screen.item} busy={content.busy} onBack={() => setScreen({ type: 'overview' })} onCancel={async (reason) => { await content.cancel(screen.item, reason); setScreen({ type: 'overview' }); }} />;
  return <BusinessContentOverview
    items={content.items} loading={content.loading} busy={content.busy} error={content.error}
    onBack={onBack} onRetry={content.refresh}
    onCreate={(kind) => setScreen({ type: 'editor', item: null, kind })}
    onEdit={(item) => setScreen({ type: 'editor', item })}
    onArchive={content.archive} onDelete={content.removeDraft}
