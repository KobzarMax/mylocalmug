...
export function BusinessOrderQueue({
  workspace,
  transactions = false,
}: {
  workspace: Workspace;
  ...
  const items = transactions
    ...
    : state.orders.filter((order) =>
        ['needs_confirmation', 'accepted', 'preparing', 'ready'].includes(order.status),
      );
    ...
    <View style={{ gap: 14 }}>
      ...
      {!items.length ? (
        <View style={styles.empty}>
          <Text style={styles.cardTitle}>{transactions ? 'No transactions yet' : 'No active orders'}</Text>
          <Text style={styles.body}>
            {transactions
              ? 'Paid and refunded payments will appear here.'
              : 'New paid customer orders appear here for confirmation.'}
          </Text>
        </View>
      ) : (
        items.map((order) => (
          <OrderCard
            key={order.id}
            order={order}
            busy={state.busyId === order.id || refundBusy === order.id}
            canManage={canManage && !transactions}
            onTransition={(status, reason) => void state.transition(order.id, status, reason)}
            onRefund={canRefund ? (amount, reason) => void refund(order.id, amount, reason) : undefined}
          />
        ))
      )}
    </View>
