  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.title}>Tell your local story</Text>
        <Text style={styles.intro}>
          Publish shop news and dated events for customers who follow your business.
        </Text>
        <View style={styles.actions}>
          <Pressable
            accessibilityRole="button"
            disabled={props.busy}
            onPress={() => props.onCreate('news')}
            style={styles.primaryButton}
          >
            <Ionicons name="newspaper-outline" size={18} color={palette.paper} />
            <Text style={styles.primaryText}>New story</Text>
          </Pressable>
          <Pressable
            accessibilityRole="button"
            disabled={props.busy}
            onPress={() => props.onCreate('event')}
            style={styles.secondaryButton}
          >
            <Ionicons name="calendar-outline" size={18} color={palette.green} />
            <Text style={styles.secondaryText}>New event</Text>
          </Pressable>
        </View>
        ...
        {props.loading ? (
          <ContentLoading label="Loading business content…" />
        ) : props.error ? (
          <ContentError message={props.error} onRetry={props.onRetry} />
        ) : !visible.length ? (
          <EmptyContent
            title="Nothing here yet"
            message={
              filter === 'all'
                ? 'Create your first story or event.'
                : `No ${filter} content matches this filter.`
            }
          />
        ) : (
          visible.map((item) => (
            <View key={item.id}>
              ...
              <View style={styles.actions}>
                ...
                {item.kind === 'event' && item.publishedAt && !item.eventCancelledAt && (
                  <Pressable
                    accessibilityRole="button"
                    disabled={props.busy}
                    onPress={() => props.onCancel(item)}
                    style={[styles.secondaryButton, styles.warningButton]}
                  >
                    <Text style={[styles.secondaryText, styles.warningText]}>Cancel event</Text>
                  </Pressable>
                )}
                {publicationStateOf(item) === 'draft' ? (
                  <Pressable
                    ...
                    disabled={props.busy}
                    onPress={() =>
                      confirm('Delete draft?', 'This draft and its cover image will be removed.', () =>
                        props.onDelete(item),
                      )
                    }
                    style={[styles.secondaryButton, styles.warningButton]}
                  >
                    <Text style={[styles.secondaryText, styles.warningText]}>Delete</Text>
                  </Pressable>
                ) : (
                  publicationStateOf(item) !== 'archived' && (
                    <Pressable
                      accessibilityRole="button"
                      disabled={props.busy}
                      onPress={() =>
                      ...
                      style={[styles.secondaryButton, styles.warningButton]}
                    >
                      <Text style={[styles.secondaryText, styles.warningText]}>Archive</Text>
                    </Pressable>
                  )
                )}
              </View>
