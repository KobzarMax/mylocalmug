...
const allowedNodeTypes = new Set([
  ...
  'blockquote', 'hardBreak', 'codeBlock',
]);
...
export const contentEditorSchema = z.object({
  ...
  eventTimezone: z.string().trim().min(1).max(80).refine(isValidTimeZone, 'Enter a valid IANA timezone.').nullable(),
  eventVenueName: z.string().trim().max(120).nullable(),
  eventVenueAddress: z.string().trim().max(300).nullable(),
  reminderMinutes: z.array(z.union([
    z.literal(60), z.literal(1440), z.literal(10080),
  ])).max(3),
  ...
  if (value.eventAllDay) {
    ...
    if (boundaries.some((boundary) => !boundary.endsWith('T00:00:00.000Z'))) {
      context.addIssue({ code: 'custom', message: 'All-day events must use whole calendar-day boundaries.' });
    }
  }
});

export const publicationTimeSchema = z.string().datetime().refine(
...
function validateNodes(nodes: unknown[], depth: number): boolean {
  ...
  return nodes.every((candidate) => {
    ...
    return node.content === undefined || (Array.isArray(node.content) && validateNodes(node.content, depth + 1));
  });
}
...
export const REMINDER_OPTIONS: Array<{ value: EventReminderOffset; label: string }> = reminderOffsets
  .slice().reverse()
  .map((value) => ({
    value,
