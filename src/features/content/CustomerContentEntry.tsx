...
export function CustomerContentEntry({ accountId, initialContentId, onInitialContentHandled }: { accountId: string; initialContentId?: string | null; onInitialContentHandled?: () => void }) {
  ...
  if (selectedId) {
    ...
    return <ContentDetailScreen accountId={accountId} item={detail.item} isOnline={detail.isOnline} dataUpdatedAt={detail.dataUpdatedAt} onBack={() => setSelectedId(null)} onMoreFromBusiness={(id, name) => { setBusiness({ id, name }); setSelectedId(null); }} />;
  }
  return <CustomerContentFeed
    items={feed.items} loading={feed.loading} loadingMore={feed.loadingMore} error={feed.error}
    isOnline={feed.isOnline} dataUpdatedAt={feed.dataUpdatedAt}
