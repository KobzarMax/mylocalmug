...
export function PaymentsEntry({ workspace, onBack }: { workspace: Workspace; onBack: () => void }) {
  ...
  const run = (promise: Promise<unknown>) =>
    void promise.catch((error) =>
      Alert.alert('Payments', error instanceof Error ? error.message : 'Please try again.'),
    );
  return (
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {tab === 'setup' ? (
          <PaymentSetup
            ...
            onStart={(provider) => run(state.start(provider))}
            onSync={(provider) => void state.sync(provider)}
          />
        ) : null}
        {tab === 'setup' &&
        state.canConnect &&
        state.connections.some(
          (connection) => connection.provider === 'stripe' && connection.status === 'ready',
        ) ? (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Terminal location</Text>
            <Text style={styles.body}>
              Create or reuse the Stripe Terminal location mapped to the primary shop location.
            </Text>
            <Pressable
              accessibilityRole="button"
              onPress={() => run(setupTerminalLocation(workspace.business.id))}
              style={styles.button}
            >
              <Text style={styles.buttonText}>Set up Terminal location</Text>
            </Pressable>
          </View>
        ) : null}
