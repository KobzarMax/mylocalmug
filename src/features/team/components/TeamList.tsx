...
export function TeamList({
  ...
  const revoke = (invitation: TeamInvitation) =>
    Alert.alert('Revoke invitation?', `${invitation.email} will no longer be able to join.`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: () =>
          onRevoke(invitation.id).catch((caught) =>
            Alert.alert(
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {canManage && (
          <Pressable accessibilityRole="button" onPress={onInvite} style={styles.primaryButton}>
            <Text style={styles.primaryText}>Invite employee</Text>
          </Pressable>
        )}
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Invite the first employee to start collaborating.</Text>
          </View>
        ) : (
          members.map((member) => (
            <MemberRow key={member.profileId} member={member} onPress={() => onMember(member)} />
          ))
        )}
            ...
            {invitations.map((invitation) => (
              <View key={invitation.id} style={styles.card}>
                <View style={styles.avatar}>
                  <Ionicons name="mail-outline" size={20} color={palette.green} />
                </View>
                ...
                {canManage && (
                  ...
                  >
                    <Ionicons name="close" size={18} color={palette.orange} />
                  </Pressable>
                )}
              </View>
            ))}
  ...
  return (
    <Pressable accessibilityRole="button" onPress={onPress} style={styles.card}>
      ...
      <View style={styles.cardBody}>
        ...
        <Text style={[styles.cardMeta, member.status === 'suspended' && styles.statusWarning]}>
          {member.status}
        </Text>
      </View>
