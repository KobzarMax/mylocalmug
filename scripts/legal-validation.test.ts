...
import {
  ...
  normalizeLegalInput,
} from '../src/features/legal/validation';
...
for (const option of ENTITY_OPTIONS) {
  const candidate = normalizeLegalInput({
    ...
    companyNumber: ['limited_company', 'limited_liability_partnership'].includes(option.value)
      ? 'SC123456'
      : '',
...
assert.equal(
  legalDraftSchema.safeParse(normalizeLegalInput({ ...base, legalName: '', tradingName: '' })).success,
  true,
  'incomplete drafts are allowed',
);
assert.equal(
  legalSubmissionSchema.safeParse(normalizeLegalInput({ ...base, legalName: '' })).success,
  false,
  'legal name is required to submit',
);
assert.equal(
  legalSubmissionSchema.safeParse(
    normalizeLegalInput({ ...base, entityType: 'limited_company', companyNumber: '' }),
  ).success,
  false,
  'company number is required',
);
assert.equal(
  legalDraftSchema.safeParse(normalizeLegalInput({ ...base, registeredPostcode: 'NOT A POSTCODE' })).success,
  false,
  'invalid postcode is rejected',
);
assert.equal(
  legalDraftSchema.safeParse(normalizeLegalInput({ ...base, vatRegistered: true, vatNumber: '123' })).success,
  false,
  'invalid VAT number is rejected',
);
assert.equal(
  legalDraftSchema.safeParse(normalizeLegalInput({ ...base, contactPhone: 'abc' })).success,
  false,
