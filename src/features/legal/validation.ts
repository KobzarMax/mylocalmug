...
export function normalizeLegalInput(input: BusinessLegalProfileInput): BusinessLegalProfileInput {
  ...
  return {
    ...
    companyNumber: ['limited_company', 'limited_liability_partnership'].includes(entityType)
      ...
      : '',
    charityNumber:
      entityType === 'charity' ? input.charityNumber.toUpperCase().replace(/[^A-Z0-9-]/g, '') : '',
    vatRegistered,
    vatNumber: vatRegistered && vatDigits ? `GB${vatDigits}` : '',
  };
}

export const legalDraftSchema = z
  .object({
    entityType: z.enum([
      'sole_trader',
      'limited_company',
    ...
    companyNumber: optionalPattern(
      companyNumber,
      'Use an 8-digit or two-letter plus 6-digit company number.',
    ),
  ...
  .superRefine((value, context) => {
    ...
    if (!needsCompany && value.companyNumber)
      context.addIssue({
        code: 'custom',
        path: ['companyNumber'],
        message: 'Company number is not used for this entity type.',
      });
    if (value.entityType !== 'charity' && value.charityNumber)
      context.addIssue({
        code: 'custom',
        path: ['charityNumber'],
        message: 'Charity number is only used for charities.',
      });
    if (!value.vatRegistered && value.vatNumber)
      context.addIssue({
        code: 'custom',
        path: ['vatNumber'],
        message: 'Remove the VAT number or mark the business as VAT registered.',
      });
  });
...
export const legalSubmissionSchema = legalDraftSchema.superRefine((value, context) => {
  ...
  if (
    ['limited_company', 'limited_liability_partnership'].includes(value.entityType) &&
    !value.companyNumber
  ) {
    context.addIssue({
      code: 'custom',
      path: ['companyNumber'],
      message: 'Enter the Companies House number.',
    });
  }
  if (value.entityType === 'charity' && !value.charityNumber)
    context.addIssue({ code: 'custom', path: ['charityNumber'], message: 'Enter the charity number.' });
  if (value.vatRegistered && !value.vatNumber)
    context.addIssue({ code: 'custom', path: ['vatNumber'], message: 'Enter the VAT number.' });
});
