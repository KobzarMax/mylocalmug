...
export function InvitationCreatedScreen({ invitation, onBack, onDone }: { invitation: CreatedInvitation; onBack: () => void; onDone: () => void }) {
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.scroll}>
    ...
    <View style={styles.tokenCard}><Text style={styles.tokenLabel}>Single-use invitation code</Text><Text selectable style={styles.token}>{invitation.token}</Text><Text style={styles.warning}>This is the only time the full code is shown. It expires {new Date(invitation.expiresAt).toLocaleString()}.</Text></View>
