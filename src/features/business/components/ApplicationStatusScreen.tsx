      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statusHero}>
          <View style={styles.statusIcon}>
            <Ionicons
              name={underReview ? 'search' : application.status === 'approved' ? 'checkmark' : 'time'}
              size={28}
              color={colors.green}
            />
          </View>
          ...
          <Text style={styles.statusText}>
            {application.status === 'approved'
              ? 'Your workspace is being prepared. Refresh in a moment.'
              : underReview
                ? 'We’re checking your business details. You’ll see the decision here.'
                : 'Your application is safely queued for review.'}
          </Text>
        </View>
        <View style={styles.summaryCard}>
          <SummaryRow label="Business" value={application.tradingName} />
          <SummaryRow label="Address" value={application.address} />
          <SummaryRow label="Contact" value={application.contactEmail} />
        </View>
