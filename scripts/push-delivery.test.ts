...
} from '../supabase/functions/_shared/expo';
...
assert.equal(
  uniqueByKey([
    { key: 'job:device', value: 1 },
    { key: 'job:device', value: 2 },
  ]).length,
  1,
  'duplicate job/device work should collapse',
);
...
async function main() {
  ...
  assert.deepEqual(
    ...
    'successful ticket response should be returned',
  );

  const retryFetch: typeof fetch = async () => new Response('unavailable', { status: 503 });
  await assert.rejects(
    () => sendExpoMessages([message], retryFetch),
    /503/,
    'transient Expo failure should surface for retry',
  );
  await assert.rejects(
    () =>
      sendExpoMessages(
    ...
    /100/,
    'oversized batches should fail before sending',
  );
