...
export async function createOrder(
  ...
  const result = await supabase.rpc('create_trusted_order', {
  ...
  return getOrder(String(result.data.id));
}
export async function getOrder(orderId: string) {
  const result = await supabase.from('orders').select('*,order_items(*)').eq('id', orderId).single();
  if (result.error) throw result.error;
...
export async function listBusinessOrders(businessId: string) {
  const result = await supabase
    ...
    .limit(100);
  if (result.error) throw result.error;
  return (result.data as Record<string, unknown>[]).map(mapOrder);
}
...
export async function updateOrderStatus(orderId: string, status: OrderStatus, reason?: string) {
  const result = await supabase.rpc('update_order_fulfilment', {
...
export async function cancelUnpaidOrder(orderId: string) {
  const result = await supabase.rpc('cancel_unpaid_order', { target_order_id: orderId });
  if (result.error) throw result.error;
  return getOrder(orderId);
}
function mapOrder(row: Record<string, unknown>): Order {
  const raw = Array.isArray(row.order_items) ? (row.order_items as Record<string, unknown>[]) : [];
  return {
    id: String(row.id),
    ...
    refundedPence: Number(row.refunded_pence),
    confirmationDeadline: row.confirmation_deadline ? String(row.confirmation_deadline) : null,
