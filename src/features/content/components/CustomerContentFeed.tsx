...
export function CustomerContentFeed(props: {
  ...
  return <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
    ...
    <OfflineNotice isOnline={props.isOnline} updatedAt={props.dataUpdatedAt} stale={Boolean(props.error && props.items.length)} />
    {props.loading && !props.items.length ? <ContentLoading /> : props.error && !props.items.length ? <ContentError message={props.isOnline ? props.error : 'These stories were not saved before going offline.'} onRetry={props.onRetry} /> : !props.items.length ? <EmptyContent title={props.isOnline ? (props.followedOnly ? 'No stories from followed shops' : 'No published stories yet') : 'No saved stories'} message={props.isOnline ? (props.followedOnly ? 'Follow a coffee shop from a story to see its updates here.' : 'Published news and events will appear here.') : 'Connect to the internet and open stories once to keep them available offline.'} action={props.isOnline && props.followedOnly ? 'Browse all stories' : undefined} onAction={props.isOnline && props.followedOnly ? () => props.onSetFollowedOnly(false) : undefined} /> : <>
      {props.items.map((item) => <ContentCard key={item.id} item={item} onPress={() => props.onOpen(item.id)} />)}
      {props.hasMore && <Pressable disabled={props.loadingMore || !props.isOnline} onPress={props.onLoadMore} style={styles.loadMore}>{props.loadingMore ? <ActivityIndicator color={palette.green} /> : <Text style={styles.secondaryText}>{props.isOnline ? 'Load more stories' : 'Reconnect to load more'}</Text>}</Pressable>}
    </>}
  </ScrollView>;
}
